		...
		if (attackerPlayer != null && plugin.getTownyUniverse().isWarTime() && TownySettings.getWartimeDeathPrice() > 0)
			try {
				...
				if (price > 0) {
					...
					plugin.sendMsg(attackerPlayer, "You robbed " + defenderResident.getName() +" of " + EconomyTools.Format(price) + ".");
					plugin.sendMsg(defenderPlayer, attackerResident.getName() + " robbed you of " + price + " " + EconomyTools.Format(price) + ".");
				}
			...
			try {
				...
				plugin.sendMsg(defenderPlayer, "You lost " + EconomyTools.Format(price) + ".");
			} catch (EconomyException e) {
